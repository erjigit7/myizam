...
public sealed partial class MinjustHtmlParser
{
    ...
    // Кыргызские маркеры (фаза 2, НЕ проверено на реальных данных):
    // в кыргызских текстах номер идёт перед словом: «5-берене», «3-глава», «II-бөлүм».
    ...
    [GeneratedRegex(@"^(?:(?:Раздел|РАЗДЕЛ|Бөлүм|БӨЛҮМ)\s+([IVXLC]+|\d+)|([IVXLC]+|\d+)\s*-\s*(?:бөлүм|Бөлүм|БӨЛҮМ))\s*(?:\.\s*(.*))?$")]
