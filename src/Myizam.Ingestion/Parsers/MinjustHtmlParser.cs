...
public sealed partial class MinjustHtmlParser
{
    ...
    // Кыргызские маркеры — проверено на реальных кодексах (15.07.2026):
    // новые (ТК-2025, УК-2021…): «1-берене.», «1-глава.», разделы «I БӨЛҮМ. …»;
    // старые (СК-2003, ГК-1996/98): «11-статья.» (русское слово после номера!)
    // и «3-Глава Никеге туруу…» — БЕЗ точки после слова. См. docs/kg_notes.md.
    // «статъя» (Ъ) — реальная опечатка в ГК-II kg (ст. 738-36); пробелы в номере —
    // артефакт суперскриптов («738<sup>36</sup>-статья» → «738 -36-статья»)
    [GeneratedRegex(@"^(?:(?:Статья|СТАТЬЯ|Берене|БЕРЕНЕ)\s+(\d+(?:\s*-\s*\d+)*)|(\d+(?:\s*-\s*\d+)*)\s*-\s*(?:берене|Берене|БЕРЕНЕ|[Сс]тат[ьъ]я|СТАТ[ЬЪ]Я))\s*(?:\.\s*(.*))?$")]
    private static partial Regex ArticleRxKg();

    // (?!\p{L}) — граница слова: «34-1-главада каралбаган…» (падежная форма
    // в начале абзаца, реальный кейс ГК-II kg) — это ТЕКСТ статьи, не глава
    [GeneratedRegex(@"^(?:(?:Глава|ГЛАВА|Бап|БАП)\s+(\d+(?:-\d+)*)|(\d+(?:-\d+)*)\s*-\s*(?:глава|Глава|ГЛАВА|бап|Бап|БАП)(?!\p{L}))\s*\.?\s*(.*)$")]
    private static partial Regex ChapterRxKg();

    [GeneratedRegex(@"^(?:(?:Раздел|РАЗДЕЛ|Бөлүм|БӨЛҮМ)\s+([IVXLC]+|\d+)|([IVXLC]+|\d+)\s*-?\s*(?:бөлүм|Бөлүм|БӨЛҮМ|БӨЛYМ)(?!\p{L}))\s*\.?\s*(.*)$")]
    ...
    public ParsedLaw Parse(string html, string lang = "ru")
    {
        ...
        for (var i = 0; i < blocks.Count; i++)
        {
            ...
            if (m.Success)
            {
                ...
                // Word-артефакт: заголовок и тело слиты в один абзац (ГК-II kg,
                // ст. 738-58) — «заголовок» длиннее любого реального названия
                // статьи уходит в тело, иначе статья остаётся «пустой»
                if (article.Title is { Length: > 250 } mergedBody)
                {
                    article.Paragraphs.Add(mergedBody);
                    article.Title = null;
                }
